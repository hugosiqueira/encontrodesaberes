...
Ext.define('Ext.ux.upload.Basic', {
    ...
    constructor: function(owner, config)
    {
        ...
        me.actions = {
            ...
            start: Ext.create('Ext.Action', {
                text: config.uploadButtonText || 'Gravar',
                disabled: true,
                iconCls: 'icon-save',
                handler: me.start,
                scope: me
            ...
            cancel: Ext.create('Ext.Action', {
                text: config.cancelButtonText || 'Cancelar',
                disabled: true,
            ...
            removeAll: Ext.create('Ext.Action', {
				iconCls: 'icon-cancel',
                text: config.deleteAllText || 'Fechar',
                disabled: true,
    ...
    start: function(button){
		...
		if(form.isValid()) {
			if(Ext.getCmp('modProp_gridReqOrador').getSelectionModel().hasSelection()){
				var grid =  Ext.getCmp('modProp_gridReqOrador');
				console.log('gridreq');
			}
			else{
				var grid = Ext.getCmp('modProp_gridPropostas');
				console.log('gridproposta');
			}
			var row = grid.getSelectionModel().getSelection()[0];
			var me = this;
			me.fireEvent('beforestart', me);
			if(me.multipart_params){
				me.uploader.settings.multipart_params = {
					fgk_tipo_documento	: values.fgk_tipo_documento,
					nome_documento		: values.nome_documento,
					fgk_orador			: row.data.id_orador,
					fgk_proposta		: row.data.id_proposta,
					autor_documento		: values.autor_documento,
					tipo_privacidade	: values.tipo_privacidade
				};
			}
    ...
    {
        var me = this, response = Ext.JSON.decode(status.response);
    ...
    _StateChanged: function(uploader)
    {
        ...
        else
        {
            ...
            this.actions.start.setDisabled(true);
			this.actions.removeAll.setDisabled(false);
